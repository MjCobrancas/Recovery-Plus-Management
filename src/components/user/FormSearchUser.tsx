...
export function SearchUser({ allUsers, filterData, totalPage, backend_domain, token }: { allUsers: IncompleteUserValuesData, filterData: Function, totalPage: number, backend_domain: string, token: string }) {
    ...
    async function handleNextPage(nextPage: number) {
        ...
        const result: IncompleteUserValuesData = await getUsersPagination(backend_domain, ((nextPage - 1) * 24), 24, token)
        filterData(result)
    }

    async function handleBackPage(backPage: number) {
        setActualPage(backPage)
        const result: IncompleteUserValuesData = await getUsersPagination(backend_domain, ((backPage - 1) * 24), 24, token)
        filterData(result)
    }

    async function handleInitialPage() {
        setActualPage(1)
        const result: IncompleteUserValuesData = await getUsersPagination(backend_domain, (0 * 24), 24, token)
        filterData(result)
    }

    async function handleEndPage() {
        setActualPage(totalPage)
        const result: IncompleteUserValuesData = await getUsersPagination(backend_domain, ((totalPage - 1) * 24), 24, token)
        filterData(result)
    }
